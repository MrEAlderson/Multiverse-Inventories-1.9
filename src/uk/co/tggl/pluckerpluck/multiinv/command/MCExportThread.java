		...
		for(OfflinePlayer player1 : oplayers) {
			try {
				...
				if(player != null) {
					...
					ItemStack[] inventorycontents = playerinstance.getInventory().getInventoryContents();
					// Iterate and get inventory contents
					ItemStack[] inventory = new ItemStack[inventorycontents.length];
					for(int i = 0; i < inventory.length; i++) {
						if(inventorycontents[i] != null) {
							inventory[i] = inventorycontents[i];
						} else {
							inventory[i] = null;
						}
					}
					...
					ItemStack[] armorcontents = playerinstance.getInventory().getArmorContents();
					ItemStack[] armor = new ItemStack[armorcontents.length];
					for(int i = 0; i < armor.length; i++) {
						if(armorcontents[i] != null) {
							armor[i] = armorcontents[i];
						} else {
							armor[i] = null;
						}
					}
