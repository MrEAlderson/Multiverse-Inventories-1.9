...
public class MIInventoryConverter {
    ...
    public static MIItemStack[] convertISItems(ItemStack[] oldContents) {
    	MIItemStack[] newContents = MinecraftTools.fillWithAir(new MIItemStack[oldContents.length]);
        for (int i = 0; i < oldContents.length; i++) {
            if (oldContents[i] != null) {
                newContents[i] = new MIItemStack(oldContents[i]);
            }
        }
        return newContents;
    }
